...
const game = {
    ...
    for (let y = 0; y < map.length; y++) {
      ...
      for (let x = 0; x < row.length; x++) {
        ...
        if (square === "p") {
          if (game.players === 0) {
            player1 = new Player(
              x * BLOCKLENGTH + BLOCKLENGTH / 4,
              y * BLOCKLENGTH + BLOCKLENGTH / 4
            );
    ...
    game.objects.forEach((obstacle) => {
      if (obstacle.type === "obstacle") {
        ...
        if (
          ...
          object.position.y + object.velocity.y >
            obstacle.position.y - object.height
        ) {
          if (object.type === "player") {
        ...
        // bottom
          ...
          object.position.x < obstacle.position.x + obstacle.width &&
          object.position.y + object.velocity.y >
            obstacle.position.y + obstacle.height - object.height &&
          object.position.y + object.velocity.y <
        ...
        if (
          ...
          object.position.x + object.velocity.x >
            obstacle.position.x - object.width
        ) {
          if (object.type === "player") {
            object.position.x = obstacle.position.x - object.width;
            object.velocity.x = 0;
        ...
        // right
          ...
          object.position.y < obstacle.position.y + obstacle.height &&
          object.position.x + object.velocity.x >
            obstacle.position.x + obstacle.width - object.width &&
          object.position.x + object.velocity.x <
        ...
        if (
          object.position.x > player1.position.x - object.width &&
          object.position.x < player1.position.x + player1.width &&
          object.position.y > player1.position.y - object.height &&
          object.position.y < player1.position.y + player1.height
        ) {
          player1.alive = false;
        }
      }
  ...
  updateGameInfo: () => {
    ...
    getElement("#game div:nth-child(3) span").innerText =
      game.balls + "/" + game.ballgorithm();
    getElement("#game div:nth-child(4) span").innerText =
      Math.floor(game.time) + paused;
  },
};
...
document.onkeydown = (event) => {
  ...
  if (game.players < 2) {
    ...
    // move player 2
    if (keys.move_player2.up.includes(event.key)) {
      player2.move.up = true;
      getElement("#shoot .up").classList.add("active");
    }
    if (keys.move_player2.down.includes(event.key)) {
      player2.move.down = true;
      getElement("#shoot .down").classList.add("active");
    }
    if (keys.move_player2.left.includes(event.key)) {
      player2.move.left = true;
      getElement("#shoot .left").classList.add("active");
    }
    if (keys.move_player2.right.includes(event.key)) {
      player2.move.right = true;
      getElement("#shoot .right").classList.add("active");
    }
	//shoot Player 1 // THIS DOES NOT WORK!
	/////////////////////////////////////////////////////////////
	if (keys.shoot.P1.includes(event.key)) {
		player1.shoot.up = true;
		getElement("#shoot .up").classList.add("active");
	  }
	  if (keys.shoot.P1.includes(event.key)) {
...
function updateDevInfo() {
  getElement("#player div:nth-child(1) span").innerText = player1.position.x;
  getElement("#player div:nth-child(2) span").innerText = player1.position.y;
  getElement("#player div:nth-child(3) span").innerText = player1.velocity.x;
  getElement("#player div:nth-child(4) span").innerText = player1.velocity.y;
}
